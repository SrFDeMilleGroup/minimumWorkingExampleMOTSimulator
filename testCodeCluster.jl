...
molecule = ARGS[1];
aOrBOrAB = ARGS[2];
includeRepump = ARGS[3];
test = ARGS[4];
if molecule=="SrF"
    include("SrFVariables.jl")
elseif molecule=="CaF"
    include("CaFVariables.jl")
end
...
saveDataFolderTag = string(aOrBOrAB,"Mol",molecule,"Repump",includeRepump); #If you want to put anything additional in "savefoldername" to tag it, see variable folderString after lasers are declared.
addHeaders=1;
...
vRound = 0.01;#nearest normalized unit to round all speeds to.  Simulation will have periodicity of 2*pi/vRound, so if you round finely, will take a while.  Usually choose 0.02
...
#=
displacementsInMM = [5];
userSpeeds = [.5,1,1.5];#speed in xy plane (for 2d force profile) or in 3D (normalized units v/(gam/k))
...
displacementsInMM = [.1,.2,.3,.4,.5,.7,.9,1.2,1.5];
userSpeeds = [-.25,-.2,-.15,-.1,-.07,-.05,-.03,-.02,-.01,.01,.02,.03,.05,.07,.1,.15,.2,.25];
forceProfile = "ThreeD";
bFieldSetting = "ThreeD";
...
#5D) red XA 2D/3D 5-laser MOT params
#=
...
elseif test=="c"
    laserEnergy[2]=-18.5;
elseif test == "d"
    laserEnergy = laserEnergy .- 1.0;
elseif test == "e"
    laserEnergy = laserEnergy .- 0.5;
elseif test == "f"
    laserEnergy = laserEnergy .+ 0.5;
elseif test == "g"
    laserEnergy = laserEnergy .+ 1.0;
...
if molecule=="CaF"
    if evenBluer == "No"
        laserEnergy = [2,-7.3,-13.1,-16.6];
    else
        laserEnergy = [3,-6.3,-12.1,-16.6];
    end
elseif molecule =="SrF"  
    laserEnergy = [2.5,-22.];
end
...
if includeRepump == "Yes"
    append!(s0,[80.0]);
    append!(whichTransition,["XARepump"]);
    append!(polType,["3D"]);
    append!(sidebandFreqs,[6.5]);
    append!(sidebandAmps,[2.4]);
    append!(polSign,[1]);
    append!(laserEnergy,[-13.0])
end
...
for l = 1:length(displacementsInMM)
    ...
    for k=1:length(longSpeeds)
	    ...
	    if abs(currSpeed)<0.05
            ...
            tForSteadyState = maximum([2*periodLength, 270]);#obtained by trial and error.  Could potentially be handled more rigrorously (solve ode in steps of 'period length' until solution 'converges')
            ...
            @time Threads.@threads for i = 1:(numTrialsPerValueSet*2)
                currSol = sol[i]
